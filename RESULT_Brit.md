...
| 관련 최종 PR 또는 공유 링크 | https://github.com/anTuni/fornerds_study_session4/tree/defend/Brit |
...
| --- | --- | --- | --- | --- |
| 1 | `5100118` | NetrioGit | ci(defend/Brit): add security audit workflow and dependency policy | .github/dependabot.yml, .github/workflows/dependabot-auto-merge.yml, .github/workflows/security-audit.yml, cicd_practice/SECURITY_POLICY.md, cicd_practice/SECURITY_TOOLS.md |
| 2 | `993d5fe` | NetrioGit | docs(defend/Brit): add security audit result report | RESULT_defend_Brit.md |
...
| 실행 트리거 | `pull_request` to `defend/Brit`, workflow_dispatch |
| 대상 방어 브랜치 | `defend/Brit` |
| 사용한 도구 | Maven test, OWASP Dependency-Check, npm audit, Semgrep, Trivy, Gitleaks, CodeQL, Snyk |
...
| 평균 실행 시간 | OWASP Dependency-Check 약 8682초 |
| 가장 느린 단계 | OWASP Dependency-Check 약 8682초 |
...
- Maven test, OWASP Dependency-Check, npm audit, Semgrep, Trivy, Gitleaks, CodeQL, Snyk를 조합해 의존성, SAST, 시크릿, 설정 취약점을 함께 점검하도록 구성했습니다.
...
| --- | --- | --- | --- | --- | --- | --- |
| #10 | `attack/sabo` | player397 | Open | A01 Broken Access Control, A02 Cryptographic Failures, A03 Injection, A05 Security Misconfiguration, A06 Vulnerable and Outdated Components, A07 Identification and Authentication Failures, A08 Software and Data Integrity Failures, A09 Security Logging and Monitoring Failures, A10 SSRF | Fail (🚦 Security Gate, CodeQL, Aggregate Security Report, Security audit gate) | 충돌로 머지 불가 |
| #15 | `attack/tunitobrit` | anTuni | Open | A01 Broken Access Control, A03 Injection, A07 Identification and Authentication Failures | Fail (Aggregate & report, OWASP Dependency-Check, Snyk — Backend (Maven), Trivy filesystem) | 차단 추정 |
...
| 공격자 | anTuni |
| 수비자 | Brit |
| 취약점 유형 | A01 Broken Access Control, A03 Injection, A07 Identification and Authentication Failures |
| 공격 commit | 492e0df Add advanced search, preview, and health-check endpoints to ContentCo… |
| 수정 파일 | cicd_practice/backend/src/main/java/com/example/cicdpractice/content/ContentController.java |
| 취약점 설명 | PR diff의 변경 파일과 패턴 기준으로 A01 Broken Access Control, A03 Injection, A07 Identification and Authentication Failures 유형의 취약점이 포함된 것으로 판단됩니다. 구체 로그와 artifact는 Actions 링크에서 추가 확인이 필요합니다. |
...
| --- | --- |
| workflow 실행 여부 | 성공/실행됨 |
| 실행된 workflow 이름 | Security Audit, Dependabot auto-merge (patch only) |
| 실행된 check 이름 | Aggregate & report, Build & test backend, dependency-check, Build frontend, Snyk Open Source, Trivy, OWASP Dependency-Check, Snyk — Frontend (npm), Snyk — Backend (Maven), Trivy filesystem, Gitleaks (secrets), Semgrep SAST |
| 취약점 탐지 여부 | 탐지 또는 실패 check 존재 |
| 보고서 생성 여부 | 생성 또는 시도됨 |
| PR 댓글 여부 | 확인 필요 또는 미생성 |
| artifact 업로드 여부 | 생성 추정, artifact 직접 확인 필요 |
| job 실패 여부 | 실패 (Aggregate & report, OWASP Dependency-Check, Snyk — Backend (Maven), Trivy filesystem, Gitleaks (secrets), Semgrep SAST) |
| PR 머지 차단 여부 | 차단 추정 |
| 실행 시간 | OWASP Dependency-Check 약 8682초 |
| Actions run 링크 | https://github.com/anTuni/fornerds_study_session4/actions/runs/26227024466/job/77205798128 |

#### 탐지 결과 요약

```text
탐지된 파일/라인: cicd_practice/backend/src/main/java/com/example/cicdpractice/content/ContentController.java
탐지 메시지: Aggregate & report, OWASP Dependency-Check, Snyk — Backend (Maven), Trivy filesystem, Gitleaks (secrets), Semgrep SAST check가 실패했습니다.
보고서 또는 PR 댓글 요약: 자동 보고서 댓글은 확인 필요입니다.
```
...
| required status check 이름 | 미설정 |
| 실패한 check 이름 | 🚦 Security Gate, CodeQL, Aggregate Security Report, Security audit gate, Semgrep OSS, SAST - Semgrep (OWASP Top 10), Trivy - Filesystem Scan, Backend - OWASP Dependency Check, Aggregate & report, OWASP Dependency-Check |
| PR 상태 | #10: DIRTY, #15: UNSTABLE |
| 실제 머지 버튼 상태 | 확인 필요 |
...
- defend/Brit 기준 required check 상태: 미설정.
...
| 바로 적용 가능한 부분 | Maven test, OWASP Dependency-Check, npm audit, Semgrep, Trivy, Gitleaks, CodeQL, Snyk |
| 추가 실험이 필요한 부분 | branch protection, required status check, artifact/PR comment 검증, A01 custom rule |
| 다음 액션 | defend/Brit의 실패 check를 required status check로 고정하고 report job의 항상 실행 여부를 점검 |

최종 의견:

- defend/Brit 브랜치는 보안감사 workflow를 통해 여러 공격 PR의 취약점을 탐지하도록 구성되어 있습니다. 다만 실제 머지 차단은 branch protection과 required status check 설정에 좌우되므로, 현재 설정된 required check가 모든 핵심 job을 포함하는지 추가 검증하는 것이 좋습니다.
...
gh pr list --repo anTuni/fornerds_study_session4 --base defend/Brit --state all --json number,title,url,state,author,headRefName,baseRefName,createdAt,updatedAt,mergedAt,mergeStateStatus,isDraft

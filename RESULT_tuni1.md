...
| 항목 | 내용 |
| --- | --- |
| 작성 기준 브랜치 | `defend/tuni1` |
| 수비자 | tuni1 |
| 페어 | 확인 필요 |
| 작성일 | 2026-05-30 |
| 실습 저장소 | `anTuni/fornerds_study_session4` |
| 결과 문서 작성자 | Codex 자동 정리 |
| 관련 최종 PR 또는 공유 링크 | https://github.com/anTuni/fornerds_study_session4/tree/defend/tuni1 |

## 2. 방어 브랜치 이력 요약

`defend/tuni1` 브랜치에서 보안감사 workflow를 추가하거나 개선한 commit을 정리합니다.

| 순서 | Commit | 작성자 | 메시지 | 주요 변경 내용 |
| --- | --- | --- | --- | --- |
| 1 | `0121df2` | anTuni | Add defend tuni1 security audit workflow | cicd_practice/.github/workflows/security-audit.yml |
| 2 | `ad4d380` | anTuni | Move security workflow to repository root | .github/workflows/security-audit.yml |
| 3 | `4aab123` | anTuni | Reduce false positives in security audit | .github/workflows/security-audit.yml |
...
| 실행 트리거 | `pull_request` to `defend/tuni1` |
| 대상 방어 브랜치 | `defend/tuni1` |
| 사용한 도구 | Maven test, OWASP Dependency-Check, npm audit |
| 보고서 생성 방식 | artifact upload, PR comment |
| 실패 기준 | high 이상 발견 시 실패, 게이트 job에서 exit 1 |
| PR 차단 방식 | branch protection required status check로 차단 |
| required status check 이름 | Security audit gate, Security audit gate |
...
- Maven test, OWASP Dependency-Check, npm audit를 조합해 의존성, SAST, 시크릿, 설정 취약점을 함께 점검하도록 구성했습니다.
- artifact upload, PR comment 방식으로 결과를 남기도록 설계되어 PR에서 감사 결과를 추적할 수 있습니다.
- required status check(Security audit gate, Security audit gate)를 통해 실패한 보안 점검이 머지 차단으로 이어지도록 운영됩니다.

## 4. 방어 브랜치로 들어온 공격 PR 목록

`defend/tuni1`을 base로 생성된 PR을 모두 정리합니다.

| PR | 공격 브랜치 | 공격자 | 상태 | 취약점 유형 | 최종 check 결과 | 머지 차단 여부 |
| --- | --- | --- | --- | --- | --- | --- |
| #1 | `attack/tuni2` | anTuni | Open | A03 Injection, A07 Identification and Authentication Failures | Fail (Security audit gate) | 차단 |
| #14 | `attack/sabo` | player397 | Open | A01 Broken Access Control, A02 Cryptographic Failures, A03 Injection, A05 Security Misconfiguration, A06 Vulnerable and Outdated Components, A07 Identification and Authentication Failures, A08 Software and Data Integrity Failures, A09 Security Logging and Monitoring Failures, A10 SSRF | Fail (🚦 Security Gate, CodeQL, Aggregate Security Report, Security audit gate) | 차단 추정 |
...
| 항목 | 내용 |
| --- | --- |
| base 브랜치 | `defend/tuni1` |
| 공격 브랜치 | `attack/tuni2` |
| PR 링크 | https://github.com/anTuni/fornerds_study_session4/pull/1 |
| 공격자 | anTuni |
| 수비자 | tuni1 |
| 취약점 유형 | A03 Injection, A07 Identification and Authentication Failures |
| 공격 commit | c18c674 Introduce sample XSS vulnerability, b35af88 Merge branch 'defend/tuni1' into attack/tuni2, 3f96c2e Merge branch 'defend/tuni1' into attack/tuni2 |
| 수정 파일 | cicd_practice/frontend/src/App.tsx |
| 취약점 설명 | PR diff의 변경 파일과 패턴 기준으로 A03 Injection, A07 Identification and Authentication Failures 유형의 취약점이 포함된 것으로 판단됩니다. 구체 로그와 artifact는 Actions 링크에서 추가 확인이 필요합니다. |
...
| 실행된 check 이름 | Security audit gate |
| 취약점 탐지 여부 | 탐지 또는 실패 check 존재 |
| 보고서 생성 여부 | 확인 필요 |
| PR 댓글 여부 | 확인 필요 또는 미생성 |
| artifact 업로드 여부 | 생성 추정, artifact 직접 확인 필요 |
| job 실패 여부 | 실패 (Security audit gate) |
| PR 머지 차단 여부 | 차단 |
| 실행 시간 | Security audit gate 약 50초 |
| Actions run 링크 | https://github.com/anTuni/fornerds_study_session4/actions/runs/26218979254/job/77148674623 |

#### 탐지 결과 요약

```text
탐지된 파일/라인: cicd_practice/frontend/src/App.tsx
탐지 메시지: Security audit gate check가 실패했습니다.
...
| base 브랜치 | `defend/tuni1` |
| 공격 브랜치 | `attack/sabo` |
| PR 링크 | https://github.com/anTuni/fornerds_study_session4/pull/14 |
| 공격자 | player397 |
| 수비자 | tuni1 |
...
| 공격 commit | 889b290 Add security practice result form, 46c43e8 Revise result form for defend branch reports, d949b07 Add intentionally vulnerable changes for attack/sabo |
...
| --- | --- | --- | --- |
| A03 Injection | Semgrep/CodeQL/SAST | cicd_practice/frontend/src/App.tsx | 패턴 또는 알려진 CVE/시크릿 형식이 비교적 명확해 자동화 도구가 실패 check로 표면화할 수 있습니다. |
| A07 Identification and Authentication Failures | Semgrep/CodeQL/SAST | cicd_practice/frontend/src/App.tsx | 패턴 또는 알려진 CVE/시크릿 형식이 비교적 명확해 자동화 도구가 실패 check로 표면화할 수 있습니다. |
| A01 Broken Access Control | Semgrep/CodeQL/SAST | RESULT_FORM.md | 패턴 또는 알려진 CVE/시크릿 형식이 비교적 명확해 자동화 도구가 실패 check로 표면화할 수 있습니다. |
| A02 Cryptographic Failures | Gitleaks/secret scan | RESULT_FORM.md | 패턴 또는 알려진 CVE/시크릿 형식이 비교적 명확해 자동화 도구가 실패 check로 표면화할 수 있습니다. |

## 7. 탐지 실패 또는 오탐 사례

| 유형 | PR 또는 파일 | 원인 분석 | 다음 개선안 |
| --- | --- | --- | --- |
| 미탐 | 확인 필요 | 현재 조회 범위에서는 명확한 미탐 사례를 특정하지 못했습니다. | Actions artifact와 code scanning alert를 추가 대조합니다. |
...
| required status check 이름 | Security audit gate, Security audit gate |
| 실패한 check 이름 | Security audit gate, 🚦 Security Gate, CodeQL, Aggregate Security Report, Semgrep OSS, SAST - Semgrep (OWASP Top 10), Trivy - Filesystem Scan, Backend - OWASP Dependency Check |
| PR 상태 | #1: BLOCKED, #14: BEHIND |
| 실제 머지 버튼 상태 | 확인 필요 |
...
- defend/tuni1 기준 required check 상태: Security audit gate, Security audit gate.
...
| 바로 적용 가능한 부분 | Maven test, OWASP Dependency-Check, npm audit |
| 추가 실험이 필요한 부분 | branch protection, required status check, artifact/PR comment 검증, A01 custom rule |
| 다음 액션 | defend/tuni1의 실패 check를 required status check로 고정하고 report job의 항상 실행 여부를 점검 |

최종 의견:

- defend/tuni1 브랜치는 보안감사 workflow를 통해 여러 공격 PR의 취약점을 탐지하도록 구성되어 있습니다. 다만 실제 머지 차단은 branch protection과 required status check 설정에 좌우되므로, 현재 설정된 required check가 모든 핵심 job을 포함하는지 추가 검증하는 것이 좋습니다.
...
git log --oneline --decorate origin/main..origin/defend/tuni1
git log --stat origin/main..origin/defend/tuni1
git ls-tree -r --name-only origin/defend/tuni1 .github/workflows
git show origin/defend/tuni1:.github/workflows/security-audit.yml
gh pr list --repo anTuni/fornerds_study_session4 --base defend/tuni1 --state all --json number,title,url,state,author,headRefName,baseRefName,createdAt,updatedAt,mergedAt,mergeStateStatus,isDraft
...
gh api repos/anTuni/fornerds_study_session4/branches/defend/tuni1/protection
```

...
| 관련 최종 PR 또는 공유 링크 | https://github.com/anTuni/fornerds_study_session4/tree/defend/YUN |

## 2. 방어 브랜치 이력 요약

`defend/YUN` 브랜치에서 보안감사 workflow를 추가하거나 개선한 commit을 정리합니다.

| 순서 | Commit | 작성자 | 메시지 | 주요 변경 내용 |
| --- | --- | --- | --- | --- |
| 1 | `e7c6cb8` | KANGPUNGYUN | Add security audit pipeline for defend/YUN branch | .github/scripts/security_report.py, .github/workflows/security-audit.yml, scan.py |
| 2 | `88a4905` | KANGPUNGYUN | Replace environment-based admin gate with Branch Protection | .github/workflows/security-audit.yml |
| 3 | `9ae05ad` | KANGPUNGYUN | Add security audit result report for defend/YUN | RESULT_defend_YUN.md |
...
| 사용한 도구 | Maven test, OWASP Dependency-Check, npm audit, Gitleaks, custom scan.py |
...
- Maven test, OWASP Dependency-Check, npm audit, Gitleaks, custom scan.py를 조합해 의존성, SAST, 시크릿, 설정 취약점을 함께 점검하도록 구성했습니다.
...
`defend/YUN`을 base로 생성된 PR을 모두 정리합니다.

| PR | 공격 브랜치 | 공격자 | 상태 | 취약점 유형 | 최종 check 결과 | 머지 차단 여부 |
| --- | --- | --- | --- | --- | --- | --- |
| #2 | `attack/david` | handevmin | Open | A03 Injection | Pass 또는 checks 없음 | 미차단 가능 |
| #11 | `attack/sabo` | player397 | Open | A01 Broken Access Control, A02 Cryptographic Failures, A03 Injection, A05 Security Misconfiguration, A06 Vulnerable and Outdated Components, A07 Identification and Authentication Failures, A08 Software and Data Integrity Failures, A09 Security Logging and Monitoring Failures, A10 SSRF | Fail (🚦 Security Gate, CodeQL, Aggregate Security Report, Security audit gate) | 차단 추정 |
| #19 | `attack/Brit` | Brit-juho | Open | A01 Broken Access Control, A03 Injection, A06 Vulnerable and Outdated Components, A07 Identification and Authentication Failures, A08 Software and Data Integrity Failures, A10 SSRF | Pass 또는 checks 없음 | 충돌로 머지 불가 |
...
| 공격자 | handevmin |
| 수비자 | YUN |
| 취약점 유형 | A03 Injection |
| 공격 commit | 7881dfe Improve content search with native SQL for case-insensitive matching, 8874688 Trigger CI run after base branch workflow update |
| 수정 파일 | cicd_practice/backend/src/main/java/com/example/cicdpractice/content/ContentRepository.java |
| 취약점 설명 | PR diff의 변경 파일과 패턴 기준으로 A03 Injection 유형의 취약점이 포함된 것으로 판단됩니다. 구체 로그와 artifact는 Actions 링크에서 추가 확인이 필요합니다. |
...
| 실행된 check 이름 | 🚦 Security Gate, 🔐 Admin Approval Required, 📋 Security Audit Report, 🔑 Secret Scanning (Gitleaks), 🔍 Custom Security Scan (scan.py), 📦 Dependency Vulnerability Scan |
| 취약점 탐지 여부 | 확인 필요 |
| 보고서 생성 여부 | 생성 또는 시도됨 |
| PR 댓글 여부 | 확인 필요 또는 미생성 |
| artifact 업로드 여부 | 생성 추정, artifact 직접 확인 필요 |
| job 실패 여부 | 실패 check 확인 안 됨 |
| PR 머지 차단 여부 | 미차단 가능 또는 확인 필요 |
| 실행 시간 | 📦 Dependency Vulnerability Scan 약 3854초 |
| Actions run 링크 | https://github.com/anTuni/fornerds_study_session4/actions/runs/26226542524/job/77187294541 |

#### 탐지 결과 요약

```text
탐지된 파일/라인: cicd_practice/backend/src/main/java/com/example/cicdpractice/content/ContentRepository.java
탐지 메시지: 🚦 Security Gate, 🔐 Admin Approval Required, 📋 Security Audit Report, 🔑 Secret Scanning (Gitleaks), 🔍 Custom Security Scan (scan.py) check는 성공했고 실패 여부는 추가 확인이 필요합니다.
...
| 실행 시간 | 확인 필요 |
| Actions run 링크 | https://github.com/anTuni/fornerds_study_session4/pull/19/checks |
...
| --- | --- | --- | --- |
| A03 Injection | Semgrep/CodeQL/SAST | cicd_practice/backend/src/main/java/com/example/cicdpractice/content/ContentRepository.java | 패턴 또는 알려진 CVE/시크릿 형식이 비교적 명확해 자동화 도구가 실패 check로 표면화할 수 있습니다. |
...
| --- | --- | --- | --- |
| 미탐/확인 필요 | PR #2 | 실패 check가 명확히 확인되지 않아 실제 탐지 여부를 artifact에서 확인해야 합니다. | security gate가 항상 실행되도록 needs/if 조건을 점검하고 custom rule을 추가합니다. |
...
| required status check 이름 | 미설정 |
| 실패한 check 이름 | 🚦 Security Gate, CodeQL, Aggregate Security Report, Security audit gate, Semgrep OSS, SAST - Semgrep (OWASP Top 10), Trivy - Filesystem Scan, Backend - OWASP Dependency Check |
| PR 상태 | #2: CLEAN, #11: UNSTABLE, #19: DIRTY |
| 실제 머지 버튼 상태 | 확인 필요 |
...
- defend/YUN 기준 required check 상태: 미설정.
...
| 바로 적용 가능한 부분 | Maven test, OWASP Dependency-Check, npm audit, Gitleaks, custom scan.py |
| 추가 실험이 필요한 부분 | branch protection, required status check, artifact/PR comment 검증, A01 custom rule |
| 다음 액션 | defend/YUN의 실패 check를 required status check로 고정하고 report job의 항상 실행 여부를 점검 |

최종 의견:

- defend/YUN 브랜치는 보안감사 workflow를 통해 여러 공격 PR의 취약점을 탐지하도록 구성되어 있습니다. 다만 실제 머지 차단은 branch protection과 required status check 설정에 좌우되므로, 현재 설정된 required check가 모든 핵심 job을 포함하는지 추가 검증하는 것이 좋습니다.
...
git ls-tree -r --name-only origin/defend/YUN .github/workflows
git show origin/defend/YUN:.github/workflows/security-audit.yml
gh pr list --repo anTuni/fornerds_study_session4 --base defend/YUN --state all --json number,title,url,state,author,headRefName,baseRefName,createdAt,updatedAt,mergedAt,mergeStateStatus,isDraft
...
gh api repos/anTuni/fornerds_study_session4/branches/defend/YUN/protection
```

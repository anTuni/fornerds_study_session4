...
| 관련 최종 PR 또는 공유 링크 | https://github.com/anTuni/fornerds_study_session4/tree/defend/sabo |

## 2. 방어 브랜치 이력 요약

`defend/sabo` 브랜치에서 보안감사 workflow를 추가하거나 개선한 commit을 정리합니다.

| 순서 | Commit | 작성자 | 메시지 | 주요 변경 내용 |
| --- | --- | --- | --- | --- |
| 1 | `39eb060` | player397 | Add security audit workflow for defend/sabo PRs | .github/workflows/security-audit.yml |
| 2 | `ae8dcb0` | player397 | Add Snyk scan with markdown report and gate build on security jobs | .github/scripts/snyk-to-markdown.py, .github/workflows/security-audit.yml, cicd_practice/docs/security-tools.md |
| 3 | `3ee9d29` | player397 | Drop Snyk, aggregate OSS scans into a single security report | .github/scripts/security-report-to-markdown.py, .github/scripts/snyk-to-markdown.py, .github/workflows/security-audit.yml, cicd_practice/docs/security-tools.md |
...
| workflow 이름 | Security Audit (defend/sabo) |
| 실행 트리거 | `pull_request` to `defend/sabo` |
| 대상 방어 브랜치 | `defend/sabo` |
| 사용한 도구 | Maven test, OWASP Dependency-Check, npm audit, Semgrep, Trivy, Gitleaks |
| 보고서 생성 방식 | artifact upload, PR comment, SARIF/code scanning |
| 실패 기준 | high 이상 발견 시 실패, critical 발견 시 실패, Dependency-Check CVSS 임계값 초과 시 실패 |
| PR 차단 방식 | branch protection required status check로 차단 |
| required status check 이름 | 미설정 |
| 평균 실행 시간 | OWASP Dependency-Check (Maven) 약 15699초 |
| 가장 느린 단계 | OWASP Dependency-Check (Maven) 약 15699초 |
...
- Maven test, OWASP Dependency-Check, npm audit, Semgrep, Trivy, Gitleaks를 조합해 의존성, SAST, 시크릿, 설정 취약점을 함께 점검하도록 구성했습니다.
...
`defend/sabo`을 base로 생성된 PR을 모두 정리합니다.

| PR | 공격 브랜치 | 공격자 | 상태 | 취약점 유형 | 최종 check 결과 | 머지 차단 여부 |
| --- | --- | --- | --- | --- | --- | --- |
| #8 | `attack/woosung` | woosung-dev | Open | A01 Broken Access Control, A02 Cryptographic Failures, A05 Security Misconfiguration, A07 Identification and Authentication Failures | Fail (OWASP Dependency-Check (Maven), Semgrep SAST, Trivy filesystem scan) | 차단 추정 |
| #21 | `attack/Brit` | Brit-juho | Open | A01 Broken Access Control, A03 Injection, A06 Vulnerable and Outdated Components, A07 Identification and Authentication Failures, A08 Software and Data Integrity Failures, A10 SSRF | Pass 또는 checks 없음 | 충돌로 머지 불가 |
...
| 공격자 | woosung-dev |
| 수비자 | sabo |
| 취약점 유형 | A01 Broken Access Control, A02 Cryptographic Failures, A05 Security Misconfiguration, A07 Identification and Authentication Failures |
| 공격 commit | 2a49f06 Refactor security config and content access control, 703f97b feat: add session lifecycle endpoints, comment moderation, profile se… |
...
| 취약점 설명 | PR diff의 변경 파일과 패턴 기준으로 A01 Broken Access Control, A02 Cryptographic Failures, A05 Security Misconfiguration, A07 Identification and Authentication Failures 유형의 취약점이 포함된 것으로 판단됩니다. 구체 로그와 artifact는 Actions 링크에서 추가 확인이 필요합니다. |
...
| 실행된 check 이름 | PR security summary, Aggregate security report, Backend build (mvn package), Frontend build (vite), Backend test (mvn test), Frontend test (install + lint), OWASP Dependency-Check (Maven), npm audit, Gitleaks secret scan, Semgrep SAST, Trivy filesystem scan |
| 취약점 탐지 여부 | 탐지 또는 실패 check 존재 |
| 보고서 생성 여부 | 생성 또는 시도됨 |
| PR 댓글 여부 | 확인 필요 또는 미생성 |
| artifact 업로드 여부 | 생성 추정, artifact 직접 확인 필요 |
| job 실패 여부 | 실패 (OWASP Dependency-Check (Maven), Semgrep SAST, Trivy filesystem scan) |
| PR 머지 차단 여부 | 차단 추정 |
| 실행 시간 | OWASP Dependency-Check (Maven) 약 15699초 |
| Actions run 링크 | https://github.com/anTuni/fornerds_study_session4/actions/runs/26227009515/job/77228530403 |

#### 탐지 결과 요약

```text
탐지된 파일/라인: cicd_practice/backend/src/main/java/com/example/cicdpractice/auth/AuthController.java, cicd_practice/backend/src/main/java/com/example/cicdpractice/auth/AuthService.java, cicd_practice/backend/src/main/java/com/example/cicdpractice/config/SecurityConfig.java, cicd_practice/backend/src/main/java/com/example/cicdpractice/content/ContentController.java, cicd_practice/backend/src/main/java/com/example/cicdpractice/user/UserAccount.java, cicd_practice/backend/src/main/java/com/example/cicdpractice/user/UserController.java
탐지 메시지: OWASP Dependency-Check (Maven), Semgrep SAST, Trivy filesystem scan check가 실패했습니다.
...
| base 브랜치 | `defend/sabo` |
| 공격 브랜치 | `attack/Brit` |
| PR 링크 | https://github.com/anTuni/fornerds_study_session4/pull/21 |
| 공격자 | Brit-juho |
| 수비자 | sabo |
...
| 실행 시간 | 확인 필요 |
| Actions run 링크 | https://github.com/anTuni/fornerds_study_session4/pull/21/checks |
...
| --- | --- | --- | --- |
| A01 Broken Access Control | Semgrep/CodeQL/SAST | cicd_practice/backend/src/main/java/com/example/cicdpractice/auth/AuthController.java | 패턴 또는 알려진 CVE/시크릿 형식이 비교적 명확해 자동화 도구가 실패 check로 표면화할 수 있습니다. |
| A02 Cryptographic Failures | Gitleaks/secret scan | cicd_practice/backend/src/main/java/com/example/cicdpractice/auth/AuthController.java | 패턴 또는 알려진 CVE/시크릿 형식이 비교적 명확해 자동화 도구가 실패 check로 표면화할 수 있습니다. |
| A05 Security Misconfiguration | Semgrep/CodeQL/SAST | cicd_practice/backend/src/main/java/com/example/cicdpractice/auth/AuthController.java | 패턴 또는 알려진 CVE/시크릿 형식이 비교적 명확해 자동화 도구가 실패 check로 표면화할 수 있습니다. |
| A07 Identification and Authentication Failures | Semgrep/CodeQL/SAST | cicd_practice/backend/src/main/java/com/example/cicdpractice/auth/AuthController.java | 패턴 또는 알려진 CVE/시크릿 형식이 비교적 명확해 자동화 도구가 실패 check로 표면화할 수 있습니다. |

## 7. 탐지 실패 또는 오탐 사례

| 유형 | PR 또는 파일 | 원인 분석 | 다음 개선안 |
| --- | --- | --- | --- |
| 미탐/확인 필요 | PR #21 | 실패 check가 명확히 확인되지 않아 실제 탐지 여부를 artifact에서 확인해야 합니다. | security gate가 항상 실행되도록 needs/if 조건을 점검하고 custom rule을 추가합니다. |
...
| required status check 이름 | 미설정 |
| 실패한 check 이름 | OWASP Dependency-Check (Maven), Semgrep SAST, Trivy filesystem scan |
| PR 상태 | #8: UNSTABLE, #21: DIRTY |
| 실제 머지 버튼 상태 | 확인 필요 |
...
- defend/sabo 기준 required check 상태: 미설정.
...
| 바로 적용 가능한 부분 | Maven test, OWASP Dependency-Check, npm audit, Semgrep, Trivy, Gitleaks |
| 추가 실험이 필요한 부분 | branch protection, required status check, artifact/PR comment 검증, A01 custom rule |
| 다음 액션 | defend/sabo의 실패 check를 required status check로 고정하고 report job의 항상 실행 여부를 점검 |

최종 의견:

- defend/sabo 브랜치는 보안감사 workflow를 통해 여러 공격 PR의 취약점을 탐지하도록 구성되어 있습니다. 다만 실제 머지 차단은 branch protection과 required status check 설정에 좌우되므로, 현재 설정된 required check가 모든 핵심 job을 포함하는지 추가 검증하는 것이 좋습니다.
...
git ls-tree -r --name-only origin/defend/sabo .github/workflows
git show origin/defend/sabo:.github/workflows/security-audit.yml
gh pr list --repo anTuni/fornerds_study_session4 --base defend/sabo --state all --json number,title,url,state,author,headRefName,baseRefName,createdAt,updatedAt,mergedAt,mergeStateStatus,isDraft
